...
(*[ RCSID "$Id: TCP1_hostLTSScript.sml,v 1.979 2009/02/17 11:56:46 tjr22 Exp $" ]*)
...
val di3_newackstuff_def = Phase.phase 2 Define`
  ...
  di3_newackstuff tcp_sock_0 seg ourfinisacked arch rttab ifds ticks =
   ...
   (*: Check [[ack]] value is sensible, \ie, not greater than the highest sequence number
    ...
    let revert_rexmt = (mode_of cb'.tt_rexmt IN {SOME Rexmt; SOME RexmtSyn} /\
    ...
    modify_cb (\cb. cb with
      <| (*: If [[revert_rexmt]] (above) flags that a bad retransmission occured, undo the congestion avoidance changes :*)
          snd_cwnd updated_by cb.snd_cwnd_prev onlywhen revert_rexmt;
          snd_ssthresh updated_by cb.snd_ssthresh_prev onlywhen revert_rexmt;
          snd_nxt updated_by cb'.snd_max onlywhen revert_rexmt;
          t_badrxtwin updated_by TimeWindowClosed onlywhen revert_rexmt
      |>) andThen
    modify_cb (\cb. cb with
      <|
          ...
          t_softerror updated_by NONE onlywhen IS_SOME emission_time;
          t_rttseg updated_by NONE onlywhen IS_SOME emission_time;
          ...
          snd_cwnd updated_by expand_cwnd cb.snd_ssthresh tcp_sock_0.cb.t_maxseg
        ...
        (*: Received a duplicate acknowledgement: it is an old acknowledgement (strictly less than
         ...
         let t_dupacks' = cb.t_dupacks + 1 in
          ...
          if t_dupacks' < 3  then
            ...
            modify_cb (\cb'.cb' with <| t_dupacks := t_dupacks' |>) andThen
            cont

          else if t_dupacks' > 3 \/ (t_dupacks' = 3 /\ TCP_DO_NEWRENO /\ ack < cb.snd_recover) then
            ...
            modify_cb (\cb'.cb' with <| t_dupacks := if t_dupacks' = 3 then 0 (*: false retransmit,
          ...
          else if t_dupacks' = 3 /\ ~(TCP_DO_NEWRENO /\ ack < cb.snd_recover) then
           (*: If this is the 3rd duplicate segment and if the host supports NewReno extensions, a
            ...
            (*: Update the control block before the retransmit to reflect which data requires
                retransmission :*)
            modify_cb (\cb'.cb' with <| t_dupacks    := t_dupacks';  (*: increment the counter :*)
            ...
            modify_cb (\cb'.cb' with <| snd_cwnd     := cb'.snd_ssthresh + cb.t_maxseg * t_dupacks';
                        ...
                        <| rcvq := rcvq';  (*: the updated receive queue :*)
                           cb := s.cb with
                             <| (*: Start the delayed ack timer if decided to earlier, \ie,
                                    [[delay_ack = T]]. :*)
                                tt_delack updated_by SOME (Timed((),fast_timer TCPTV_DELACK)) onlywhen delay_ack;
...
:*)
   ...
   <==
     ...
     p1 IN autobind(ps1,(proto_of pr),h,socks) /\
     bound = sid::h0.bound /\
...
@section [[connect]] ALL [[connect()]]
 \[ <[connect: fd * ip * port option -> unit]> \]
  ...
  successfully or not. On Linux or BSD, unsetting the [[O_NONBLOCK]] flag for [[fd]] and then calling
...
:*)
   ...
   (!h h0 h' ts_ socks tid d oq oq' cb' cb'' cb''' es es' es'' t'
     ...
     iss tf_req_tstmp' rcv_wnd request_r_scale advmss advmss' st'.
...
*)
     ...
     tf_req_tstmp' IN {F; T} /\ (*: do timestamp? :*)
     ...
     cb' = cb with <| tt_rexmt          := start_tt_rexmtsyn h.arch 0 F cb.t_rttinf;
...
/\
   ...
   <==
     ...
     h.ts = FUPDATE ts (tid,Timed(Run,d)) /\
     h.socks = socks |++ [(sid,sock with <| is1 := NONE; ps1 := ps1 |>)] /\
...
/\
   ...
   <==
     ...
     TCP_PROTO(tcp_sock) = (h.socks ' sid).pr /\
     ff.b(O_NONBLOCK) = F /\
     (linux_arch h.arch \/ bsd_arch h.arch) /\
...
/\
   ...
   (!h ts_ tid d sid fid ff ret h0
     sf es udp p1 fd socks cantsndmore cantrcvmore ps1.
   ...
   <==
     ...
     fid = h.fds ' fd /\
     FAPPLY h.files fid = File(FT_Socket(sid),ff) /\
     (ps1 = NONE \/ (ps1 = SOME p1 /\ p1 IN autobind(NONE,PROTO_UDP,h0,h0.socks))) /\
     (if linux_arch h.arch then ret = OK()
      else                      ret = (FAIL EAFNOSUPPORT))
   ...
   (*:
    ...
    REMARK these UDP rules could certainly be improved
...
:*)
   ...
   <==
     ...
     (*: REMARK use of [[LEAST P]] should be guarded with [[EXISTS P]] :*)
...
/\
   ...
   <==
     ...
     fid = h.fds ' fd /\
     n >= 0 /\
     (*: REMARK this rule is incorrect; it doesn't mean what the author thought it meant :*)
...
(!h ts tid d
    ...
    h with <|
        ts := FUPDATE ts (tid,Timed(Send2(sid,NONE,str,opts),d));
        socks := socks |++
            [(sid,Sock(SOME fid,sf,SOME i1,SOME p1,SOME i2,SOME p2,NONE,F,cantrcvmore,
                TCP_Sock(st,cb,NONE,sndq,sndurp,rcvq,rcvurp,iobc)))] |>
    -- Lh_tau -->
    h with <|
        ts := FUPDATE ts (tid,Timed(Ret (OK (IMPLODE str'')),sched_timer));
        socks := socks |++
            [(sid,Sock(SOME fid,sf,SOME i1,SOME p1,SOME i2,SOME p2,NONE,F,cantrcvmore,
                TCP_Sock(st,cb,NONE,sndq ++ str',sndurp',rcvq,rcvurp,iobc)))] |>

   <==

     st IN {ESTABLISHED; CLOSE_WAIT} /\
     space IN send_queue_space (sf.n(SO_SNDBUF))
         (LENGTH sndq) (MSG_OOB IN opts)
         h.arch cb.t_maxseg i2 /\
     space >= LENGTH str /\
     str'=str /\ str''=[] /\
     sndurp' =
         if MSG_OOB IN opts then SOME (LENGTH (sndq ++ str') - 1)
	 else sndurp
...
:*)
  ...
  (!h socks socks'
    ...
    rcvbufsize' sndbufsize' req_ws snd_cwnd'
  ...
  deliver_in_1 /* rp_tcp, network nonurgent (*: Passive open: receive SYN, send SYN,ACK :*) */
   ...
   <==
    ...
    cb' = cb with <|
              ...
              tt_rexmt          := start_tt_rexmt h.arch 0 F cb.t_rttinf;
              iss               := iss;
              irs               := seq;
              rcv_wnd           := rcv_window;
...
/\
   (!h socks iq iq' oq oq' seg sid fid sf i1 p1 i2 p2 cb es cantsndmore cantrcvmore
    ...
    bw_delay_product_for_rt rcvbufsize' sndbufsize' rcv_window data_deoobed
    data' rcv_nxt' rcv_wnd' rcv_up' tcp_sock FIN' cantrcvmore' snd_cwnd'
    tf_doing_ws'.
  ...
  <==
    ...
    (?win_ ws_ urp_ mss_ PSH_discard.
     ...
     tf_rcvd_tstmp' = IS_SOME ts /\
     tf_doing_tstmp' = (tf_rcvd_tstmp' /\ cb.tf_req_tstmp) /\
    ...
    rcv_window = calculate_bsd_rcv_wnd sf' tcp_sock /\
     ...
     cb' = cb with <|
              ...
              snd_wnd     := win (*<< snd_scale'*);
                 (* this is a SYN segment, so window scaling is ignored *)
              ...
              tf_doing_tstmp := tf_doing_tstmp' (* hmm, maybe this member should be called that not that *)
           |> /\
...
:*)
   (!h socks sid sock sock' cb cb' cb'' oq oq' shift d
     snd_cwnd_prev' snd_ssthresh_prev' t_badrxtwin' tf_req_tstmp'
   ...
   <==
     ...
     (if shift+1 >= TCP_MAXRXTSHIFT then
       ...
       cb' = cb with <|     (*: Restart the [[rexmt]] timer to time the retransmitted SYN :*)
                        ...
                        snd_recover       := cb.iss + 1;  (* = cb.snd_max *) (*: value after sending SYN :*)
       ...
       (?i1 i2 p1 p2. (sock.is1,sock.is2, sock.ps1, sock.ps2) = (SOME i1,SOME i2,SOME p1,SOME p2) /\
        ...
        enqueue_or_fail F h.arch h.rttab h.ifds [TCP seg'] oq
                        (cb with <| snd_nxt := cb.iss; tt_delack := NONE;
   ...
   <==
     ...
     (if shift+1 > (if tcp_sock.st = SYN_RECEIVED then TCP_SYNACKMAXRXTSHIFT
       ...
       let t_rttinf' = (if shift+1 > TCP_MAXRXTSHIFT DIV 4 then
        ...
        cb' = cb with <| tt_rexmt          := start_tt_rexmt h.arch (shift + 1) F cb.t_rttinf;  (*: reset to next backoff point :*)
...
/\
   ...
   <==
     ...
     sock.pr = TCP_PROTO(tcp_sock) /\
     sock'.pr = TCP_PROTO(tcp_sock') /\
     tcp_sock.cb.tt_delack = SOME (Timed((),d)) /\
     timer_expires d /\
     let sock0 = sock with <| pr := TCP_PROTO(tcp_sock with <| cb := tcp_sock.cb with <| tt_delack := NONE |> |>) |> in
     tcp_output_really h.arch F (ticks_of h.ticks) h.ifds sock0 (sock',outsegs') /\
     enqueue_or_fail_sock (tcp_sock'.st IN {CLOSED;LISTEN;SYN_SENT}) h.arch h.rttab h.ifds
                          outsegs' oq sock0 sock' (sock'',oq')
               ...
               (if tcp_sock.cb.snd_una <= icmp_seq /\ icmp_seq < tcp_sock.cb.snd_max then (* tcp_mtudisc() *)
                   if nextmtu = NONE then
                       ...
                       let mss = MIN (sock.sf.n(SO_SNDBUF))
                           (rounddown MCLBYTES
                            (THE nextmtu - 40 - (if tcp_sock.cb.tf_doing_tstmp then 12 else 0))) in
...
:*)
   ...
   <==
     ...
     let tr' = (flav,sid,quad,st,cb) in
     (* the following line should never have been added and is now gone: *)
     (* with <| snd_wnd := (cb.snd_wnd << cb.snd_scale) |> *)
