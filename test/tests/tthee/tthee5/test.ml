...
let _ = set_debug_level debug_VER;;
...
let initial_host = INITIAL_HOST_KURT in
...
let slurp_filter = create_filter kurt_priv_ip psyche_priv_ip in
let slurp_hostip = Some(hol_ip (get_initial_host_ip initial_host)) in
let slurp_iface = "eth0" in
...
(* Create processes *)
let inj_hnd = injector_create alan_remote (cmd_tcp_create alan_priv_ip 5004) in
let libd_hnd = libd_create kurt_remote (cmd_tcp_create kurt_priv_ip 5005) libd_log true true in
let merger_hdr_msg = (initial_host_to_string initial_host (uint 0) ARCH_LINUX false []) ^
  "(* Injector: running on john *)" in
merger_begin merge_hnd (Int64.of_int 100000) merger_hdr_msg;
let slurp_hnd = slurper_create kurt_remote slurp_log slurp_iface slurp_filter slurp_hostip in
...
let slurper_callback msg =
  match msg with
    TCPMSG(m) -> (
      if (m.is1 = Some(hol_ip kurt_priv_ip)) &&
...
let mylibcall = (tid_of_int_private 0,
		 BIND(fd, Some(Ocamllib.ip_of_string (string_ip kurt_priv_ip)),
		      Some(Ocamllib.port_of_int base_port))) in
let ret = libd_call libd_hnd mylibcall in

(* Put the socket into listening mode *)
let mylibcall = (tid_of_int_private 0, LISTEN(fd, 5)) in
let ret = libd_call libd_hnd mylibcall in
