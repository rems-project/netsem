...
/* $Id: sockopt.c,v 1.1 2002/08/02 14:15:19 smb50 Exp $ */
...
static int ip_sockopt_bool[] = { IP_HDRINCL };
...
CAMLprim value unix_getsockopt_bool(value socket, value option) {
  return Val_bool(Int_val(getsockopt_int(sockopt_bool, socket, SOL_SOCKET, option)));
}
...
CAMLprim value unix_ip_getsockopt_bool(value socket, value option) {
  return Val_bool(Int_val(getsockopt_int(ip_sockopt_bool, socket, IPPROTO_IP, option)));
}

CAMLprim value unix_ip_setsockopt_bool(value socket, value option, value status)
{
  return setsockopt_int(ip_sockopt_bool, socket, IPPROTO_IP, option, status);
}
