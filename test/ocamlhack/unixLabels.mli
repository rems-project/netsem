...
(* $Id: unixLabels.mli,v 1.1 2002/08/02 14:15:19 smb50 Exp $ *)
...
type ip_socket_bool_option =
    IP_HDRINCL	   (** User includes IP header *)
(** The socket options that can be consulted with {!Unix.ip_getsockopt_bool}
   and modified with {!Unix.ip_setsockopt_bool}. These options have a
   boolean value. *)
...
external ip_getsockopt : file_descr -> ip_socket_bool_option -> bool = "unix_ip_getsockopt_bool"
(** Return the current status of a boolean-valued option in the
   given socket at the IP protocol layer *)

external ip_setsockopt : file_descr -> ip_socket_bool_option -> bool -> unit = "unix_ip_setsockopt_bool"
(** Set or clear a boolean-valued option in the given socket at the IP protocol 
   layer. *)
