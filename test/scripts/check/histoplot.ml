...
*)
  ...
  let usage_string = "Histogram builder\n" ^
    "histoplot [options] -o output_file  input_file" in
...
let rec readdata acc mymin mymax line =
  try
    ...
    readdata (x::acc) (min x mymin) (max x mymax) (line+1)
  with
    End_of_file ->
      (acc,mymin,mymax)


let (data_rev,datamin,datamax) = (readdata [] infinity neg_infinity 0)
...
let total = List.fold_left (+.) 0.0 data
let number =  List.length data
let mean = total /. (float_of_int number)
